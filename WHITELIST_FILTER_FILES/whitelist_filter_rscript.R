...
gList<-fread("/scratch/u/pauer/MDS_CHIP_mCA/WHITELIST_FILTER_FILES/NEJM_2017_genes_01262020.txt")
whitelist.mis<-fread("/scratch/u/pauer/MDS_CHIP_mCA/WHITELIST_FILTER_FILES/CHIP_missense_vars_agb_01262020.txt")
whitelist.splice<-fread("/scratch/u/pauer/MDS_CHIP_mCA/WHITELIST_FILTER_FILES/CHIP_splice_vars_agb_01262020.txt")
whitelist.LoF<-fread("/scratch/u/pauer/MDS_CHIP_mCA/WHITELIST_FILTER_FILES/CHIP_nonsense_FS_vars_agb_01262020.txt")

args <- commandArgs(trailingOnly=TRUE)
sample_id <- args[2]
out_dir <- as.character(args[3])

vars<-fread(paste0(out_dir, "/", sample_id, ".hg38_multianno.txt"))
...
#write out files
write.csv(check_vars,paste(out_dir, "/", sample_id, ".annovar.varsOI.varcount.csv", sep=""), row.names=FALSE)
write.csv(varsOI.func2,paste(out_dir, "/", sample_id, ".annovar.varsOI.allvariants.csv", sep=""), row.names=FALSE)
write.csv(varsOI.func2[whitelist==T,],paste(out_dir, "/", sample_id, ".annovar.varsOI.wl.csv", sep=""), row.names=FALSE)
write.csv(varsOI.func[manualreview==T,],paste(out_dir, "/", sample_id, ".annovar.varsOI.manualreview.csv", sep=""), row.names=FALSE)
